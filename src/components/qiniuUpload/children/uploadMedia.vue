    ...
    <div class="wrap">
        <div class="bd">
            ...
            <aside class="aside" ref="$aside">
                <h3 class="title" ref="$title">已上传的视频</h3>
                <div class="log-wrap">
                    <ul class="log-list">
                        <li class="item" v-for="(item, index) in fileName" :key="index" >
                            <p><a :href="item.src" target="_blank">{{item.name}} (视频)</a></p>
                            <video width="100%" controls :src="`${item.src}?t=${index}`">
                                <!-- <source :src="`${item.src}?t=${index}`" :type='`${item.type}`' /> -->
                            </video>
                        </li>
                    </ul>
                </div>
