	...
	<div class="wrap">
		<div class="bd">
			...
			<aside class="aside" ref="$aside">
				...
				<div class="log-wrap">
					<ul class="log-list">
						<li class="item" :class="{ figure: item.type.indexOf('application') === -1 }" v-for="(item, index) in fileName" :key="index">
							<div v-if="item.type.indexOf('application') === -1"><img :src="item.src" />{{ item.name }} (图片)</div>
							<span v-else :href="item.src"
								>Excel：<a :download="item.name">{{ item.name }} (文档)</a></span
							>
						</li>
					</ul>
				</div>
