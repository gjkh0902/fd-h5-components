            ...
            <div class="upload-box">
                <div class="upload-conf">
                    ...
                    <mt-cell title="自动生成文件名">
                        <mt-switch 
                            v-model="uniqueNames"
                            @change="changeUniqueNames"></mt-switch>
                    </mt-cell>
                    <mt-cell title="文件最大体积(m)">
                        ...
                        <mt-field 
                            ...
                            :step="0.01"
                            :disableClear="!0"
                            @change="val => setOption('max_file_size', `${val}mb`)"></mt-field>
                    ...
                    <mt-cell title="文件最大数量">
                        ...
                        <mt-field 
                            ...
                            :value.sync="maxFiles"
                            :disableClear="!0"
                            @change="val => setOption('max_files', val)"></mt-field>
                    </mt-cell>
                </div>
                <button class="btn-upload" id="upload">
                    <div 
                        v-if="uploadStatus === 2 && uploadProgressText > 0"
                        class="percent" >
                        <span :style="{width: `${uploadProgressText}%`}"><em>{{uploadProgressText}} %</em></span>
                    </div>
                    <span v-else>上传</span>
                </button>
            </div>
            <aside class="aside" ref="$aside">
                <h3 class="title" ref="$title">已上传的图片、Excel</h3>
                <div class="log-wrap">
                    <ul class="log-list">
                        <li 
                            class="item" 
                            :class="{figure: item.type.indexOf('application') === -1}" 
                            v-for="(item, index) in fileName" 
                            :key="index">
                            <div v-if="item.type.indexOf('application') === -1 "><img :src="item.src" />{{item.name}} (图片)</div>
                            <span v-else :href="item.src">Excel：<a  :download="item.name">{{item.name}} (文档)</a></span>
                        </li>
                    </ul>
                </div>
            </aside>
...
export default {
    ...
    mounted () {
        this.uploader = QNUploader({
            ...
            file_type:              'image, excel',                          // 上传文件类型,默认为图片
...
.wrap {
    ...
    .bd {
        ...
        .aside {
            ...
            // .flex-align(flex-start);
            margin: .2667rem 0 0;
            padding: .4267rem .4267rem .2667rem;
            ...
            .log-list {
                ...
                .item.figure {
                    padding: .2667rem 0 .2667rem 2.1333rem;
                    height: 2.1333rem;
                }
            }
        }
