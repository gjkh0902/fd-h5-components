<template>
	<div class="process-circle-container" :style="styleContainer">
		<canvas
			class="process-circle-canvas"
			ref="canvasCircle"
			:width="c_size"
			:height="c_size"
			:style="styleCanvas"
		></canvas>
		<slot name="default" :current="current"></slot>
	</div>
</template>
...
export default {
	...
	methods: {
		...
		writeText(context, n) {
			...
			context.fillText(n.toFixed(this.toFixed) + '%', this.cirX - 55, this.cirY + 10) // 绘制实心
